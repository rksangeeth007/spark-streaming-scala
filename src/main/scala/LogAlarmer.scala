...
object LogAlarmer {
  ...
  def main(args: Array[String]) {
    ...
    //Addon challenge
    //    val windowLength = new Duration(args(0).toLong)
    //    val host = args(1)
    //    val port = args(2).toInt
    //    val maxErrorRatio = args(3)
    ...
    val statusCounts = successFailure.countByValueAndWindow(Seconds(10), Seconds(1))

    //Addon -> Using the params sent from main and also needed alert only once every 30 mins, thereby changed the sliding interval.
    //    val statusCounts = successFailure.countByValueAndWindow(windowLength, Seconds(1))
    ...
    statusCounts.foreachRDD((rdd, time) => {
      ...
      //      else {
      //        println("No data received in last 30 mins. Check for any issues on the server ")
      //      }
      ...
      if (totalError + totalSuccess > 100) {
        ...
        if (ratio > 0.5) {
          //        Addon challenge
          //        if (ratio > maxErrorRatio.toDouble) {
