...
object AverageTweetLength {
  ...
  def main(args: Array[String]) {
    ...
    var maxTweetLengthperRDD = new AtomicLong(0)
    ...
    lengths.foreachRDD((rdd, time) => {
      ...
      if (count > 0) {
        ...
        //Addon qn: Find the longest tweet length instead of average. Did myself :)
        maxTweetLengthperRDD.getAndSet(rdd.reduce((p, q) => if (p > q) p else q))
        if(maxTweetLengthperRDD.get() > maxTweetLength.get()){
          maxTweetLength.set(maxTweetLengthperRDD.get())
        }
        println("Longest tweet length: " + maxTweetLength.get())
      }
    })
...
//    ssc.checkpoint("C:/checkpoint/")
    ssc.checkpoint("/Users/s0h0902/BigDataFinal/Repos/spark_streaming_checkpoint/")
    ssc.start()
